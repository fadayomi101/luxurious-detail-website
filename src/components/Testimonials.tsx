  ...
  return (
    <section id="testimonials" className="py-10 md:py-24 bg-white text-gray-900 px-6 md:px-12 lg:px-24">
      <div className="max-w-[1400px] mx-auto">
        ...
        <div className="relative mt-10 md:mt-12 w-full overflow-hidden group py-4">
          {/* Edge gradients */}
          ...
          <div className="flex flex-col gap-4 md:gap-5">
            ...
            <div className="flex gap-4 md:gap-5 w-max animate-marquee hover:[animation-play-state:paused] pr-4 md:pr-5">
              {[...TESTIMONIALS, ...TESTIMONIALS, ...TESTIMONIALS, ...TESTIMONIALS].map((item, index) => (
                <div
                  key={`row1-${index}`}
                  className="w-[300px] md:w-[380px] bg-white border border-gray-200/80 rounded-[1.5rem] p-6 md:p-8 text-left relative flex flex-col justify-between shadow-[0_8px_30px_rgba(0,0,0,0.08)] hover:shadow-[0_8px_40px_rgba(0,0,0,0.12)] hover:border-[#CD1C24]/30 transition-all shrink-0 cursor-default"
                >
                  <Quote className="absolute top-6 right-6 w-8 h-8 text-gray-100 transition-colors duration-500" />
                  <div className="space-y-4 relative z-10">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        <div className="w-10 h-10 rounded-full bg-gray-50 border border-gray-100 flex items-center justify-center text-gray-900 font-bold font-mono text-sm shrink-0">
                          {item.name.split(' ').map((n: string) => n[0]).join('')}
                        </div>
                        <div>
                          <p className="font-bold text-sm text-gray-900 leading-none">{item.name}</p>
                          <p className="text-gray-500 font-mono text-[10px] tracking-widest mt-1">{item.vehicle}</p>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center space-x-1">
                      {[...Array(item.rating)].map((_, i) => (
                        <Star key={i} className="h-3.5 w-3.5 fill-[#CD1C24] text-[#CD1C24]" />
                      ))}
                    </div>
                    <p className="font-sans text-gray-600 text-[13px] md:text-sm leading-relaxed">
                      "{item.quote}"
                    </p>
                    <div className="pt-4 border-t border-gray-100">
                      <span className="inline-block px-2.5 py-1 bg-gray-50 border border-gray-100 text-gray-500 text-[8px] md:text-[9px] font-mono tracking-widest rounded-full uppercase">
                        {item.package}
                      </span>
                    </div>
                  </div>
            ...
            <div className="flex gap-4 md:gap-5 w-max animate-marquee-reverse hover:[animation-play-state:paused] pr-4 md:pr-5">
